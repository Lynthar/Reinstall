...
**镜像校验**：
- **Ubuntu**：完整 GPG 签名 + SHA256 校验。trans.sh 下载 `SHA256SUMS` + `SHA256SUMS.gpg`，用仓库内 `keys/ubuntu-cloud.asc` (UEC Image Automatic Signing Key, fingerprint `D2EB44626FDDC30B513D5BB71A5D6C4C7DB87C81`) 验证签名，再校验 qcow2 的 SHA256。任一步失败即中止。
- **Debian**：仅 SHA512 完整性校验（防止下载损坏）。Debian Cloud team 当前不发布 GPG 签名（`SHA512SUMS.sign`/`.asc`/`.gpg` 都不存在），所以**这步不防御 mirror 入侵**——靠的是 HTTPS 传输安全。
...
| `--web-public` | 让 web 日志监听 `0.0.0.0`（默认 `127.0.0.1`，需 SSH 端口转发查看） |
| `--no-web` | 完全禁用 web 日志查看 |
...
| 行为 | 谁会看到 |
|------|----------|
| 镜像下载 | 镜像源（`cdimage.debian.org` / `cloud-images.ubuntu.com` / `dl-cdn.alpinelinux.org` / 用户提供的 DD 镜像 URL）。Ubuntu 走 GPG 签名 + SHA256 校验；Debian 仅 SHA512 完整性校验（上游无签名） |
| GitHub API 解析 commit SHA | `api.github.com`（每次运行 1 次） |
| 安装期日志查看（HTTP `:80`） | 默认只 `127.0.0.1`，需 `ssh -L 8080:127.0.0.1:80` 转发；`--web-public` 才公网可达 |
...
- WebSocket 实时流：默认绑 `127.0.0.1:80`，用 `ssh -L 8080:127.0.0.1:80 root@SERVER` 转发后浏览器开 `http://localhost:8080`；公网开放需 `--web-public`，禁用用 `--no-web`
- 商家后台 VNC
- 串行控制台
